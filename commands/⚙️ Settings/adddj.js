...
module.exports = {
  ...
  aliases: [`adddjrole`],
  category: `⚙️ Settings`,
  description: `Let's you set the DJ role. Can have multiple DJ roles.`,
  usage: `adddj @role`,
    ...
    try {
      ...
      if (!role)
        return message.channel.send(new MessageEmbed()
          ...
          .setTitle(`${emoji.msg.ERROR} Error | Please add a role by pinging the role, @role!`)
        );
      ...
      let leftb = ``;
      if (client.settings.get(message.guild.id, `djroles`).join(``) === ``) leftb = `no Dj Roles, it means All Users are Djs`
      else
