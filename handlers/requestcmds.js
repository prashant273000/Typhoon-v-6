    ...
    if (command) //if the command is now valid
    {
      ...
      if (timestamps.has(message.author.id)) { //if the user is on cooldown
        ...
        if (now < expirationTime) { //if he is still on cooldonw
          ...
          return message.channel.send(new Discord.MessageEmbed()
            ...
            .setTitle(`<a:error:843768213308178462> Please wait ${timeLeft.toFixed(1)} more second(s) before reusing the \`${command.name}\` command.`)
      ...
      try {
        ...
        return message.channel.send(new Discord.MessageEmbed()
          .setColor(ee.wrongcolor)
          .setFooter(ee.footertext, ee.footericon)
          .setTitle("<a:error:843768213308178462> Something went wrong while, running the: `" + command.name + "` command. Try again after some time.")
