...
module.exports = {
  ...
  cooldown: 10,
  usage: "settings",
  description: "Shows you the current settings of the bot.",
  run: async (client, message, args, cmduser, text, prefix) => {
    try {
      //these lines creates the string for all botchannels
      let leftb = "";
      if (client.settings.get(message.guild.id, `botchannel`).join("") === "") leftb = "No Channels, it means all Channels are Bot Channels"
      else
      ...
      let leftd = "";
      if (client.settings.get(message.guild.id, `djroles`).join("") === "") leftd = "no Dj Roles, it means All Users are Djs"
      else
      ...
      return message.channel.send(new MessageEmbed()
        ...
        .setFooter(ee.footertext, ee.footericon)
        .setTitle(`${emoji.msg.ERROR} ERROR | Cannot display settings an error has occurred!`)
